# =============================================================================
# Phase 3: SGD One-Class SVM - ICS-Appropriate Anomaly Detector
# =============================================================================
...
logger = get_logger("SGDOneClassSVM")

class SGDOneClassSVMModel:
    """
    SGD One-Class SVM for ICS Anomaly Detection.
    - Scales linearly O(n)
    - Tight boundaries around normal operational profiles
    - Precision-Recall based threshold tuning
    """

    def __init__(self, config: dict = None):
        # Default config for SGDOneClassSVM
        self.config = config or {
            "nu": 0.1,
            "random_state": 42
        }
        self.model: Optional[SGDOneClassSVM] = None
    ...
    def estimate_contamination(
        ...
        logger.info(
            f"  Estimated nu (contamination): {contamination:.4f} "
    ...
    def train(
        ...
        logger.info("=" * 55)
        logger.info("TRAINING: SGD One-Class SVM")
        logger.info(f"  Normal samples : {X_normal.shape[0]:,}")
        logger.info(f"  Features       : {X_normal.shape[1]}")
        logger.info("=" * 55)

        if y_all is not None:
            self.config["nu"] = self.estimate_contamination(y_all)
        logger.info(f"  nu param       : {self.config['nu']}")

        logger.info("[SGD-OCSVM] Fitting model...")
        self.model = SGDOneClassSVM(**self.config)
        ...
        if X_val is not None and y_val is not None:
            ...
            logger.info("  No validation set → using default threshold=0.5")

        logger.info("SGD One-Class SVM training complete.")
        return self

    def _tune_threshold(self, X_val: np.ndarray, y_val: np.ndarray) -> None:
        logger.info("[SGD-OCSVM] Threshold tuning on validation set...")
    ...
    def tune_hyperparameters(
        ...
        logger.info(f"SGD-OCSVM Hyperparameter tuning...")

        best_score = -np.inf
        best_cfg = self.config.copy()

        for i, nu in enumerate(param_grid["nu"]):
            cfg = {**self.config, "nu": nu}
            try:
                m = SGDOneClassSVM(**cfg)
                m.fit(X_normal)
                raw = m.decision_function(X_val)
                scores = -raw

                auc = roc_auc_score(y_bin, scores) if len(np.unique(y_bin)) > 1 else 0.5
                ap = average_precision_score(y_bin, scores) if len(np.unique(y_bin)) > 1 else 0.5
                ...
                logger.debug(f"  Combo failed: nu={nu} → {e}")

        logger.info(f"Best SGD-OCSVM config: {best_cfg}")
    ...
    def evaluate(self, X: np.ndarray, y_true: np.ndarray, dataset_name: str = "Test") -> Dict:
        self._assert_trained()
        ...
        logger.info(f"── SGD-OCSVM Evaluation [{dataset_name}] ──")
    ...
    def save(self, version: str = "v1") -> str:
        self._assert_trained()
        path = os.path.join(MODEL_DIR, f"sgd_one_class_svm_{version}.joblib")
        joblib.dump({
            "model": self.model,
        ...
        logger.info(f"SGD-OCSVM model saved: {path}")
        return path

    def load(self, version: str = "v1") -> "SGDOneClassSVMModel":
        path = os.path.join(MODEL_DIR, f"sgd_one_class_svm_{version}.joblib")
        ...
        self.is_trained = True
        logger.info(f"SGD-OCSVM model loaded: {path}")
        return self
